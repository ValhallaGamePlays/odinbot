...
const soundBotInfo = {username:"leinadserdna79@gmail.com",pass:"querty!",enabled:true,pic:'./icon_soundbot.png'};
...
const musicDirectory = './rpgmode/music/';
...
class RPGSimulator
{
	constructor()
	{
		...
		this.commands = [
			...
			{secret: false, id:"!move", category: "Party", description:"Moves to a square on the dungeon.", finale: function(sim,message) {
				...
				message.reply("You're now in slot "+CPP+".");
	...
	initializeBot()
	{
		...
		{
			console.log("SOUNDBOT IS READY!");
			this.soundBot.user.setAvatar(this.soundBot.pic);
		...
		this.musicBot.login(botInfo.username,botInfo.pass).then(function()
		{
			...
			this.musicBot.user.setAvatar(this.musicBot.pic);
	...
	botJoinVoice(thebot,sound)
	{
		...
		console.log("[RPG] Attempting to join "+VC.name+"'s voice...");
		
		VC.join().then(function(conn)
		{
			console.log("[RPG] Bot joined the voice.);
	...
	printInventory(chan,inventoryData)
	{
		...
		var BG = new Image(imageDirectory+IMG_invBack,"auto").loadFont(fontDirectory+'pixel_maz.ttf','Gothic').loadFont(fontDirectory+'pixtech.ttf','PixTech').draw(cxt => {
	...
	printBadge(chan,badgeInfo)
	{
		...
		// Load the image
		var BG = new Image(imageDirectory+IMG_profBack,"auto").resize(400,HT).loadFont(fontDirectory+'peepo.ttf','Peepo').loadFont(fontDirectory+'pixel-love.ttf','PixelMaz').loadFont(fontDirectory+'pixtech.ttf','PixTech').draw(cxt => {
